...
void countSort2( char **strings, int numberOfLetters, int numberOfStrings,int  digit )
{
	int *distinctLetters,i,numberOfDistinctLetters;
	distinctLetters = countDistinctLetters(strings,digit,numberOfStrings);
	numberOfDistinctLetters = 0;
	for ( i = 0; i < 26; i ++ )
	{
	...
	int latestIndex = 0,j,k;
	char *temp;
	temp = (char*) calloc ( numberOfLetters, sizeof(char));
	for ( i = 0; i < 26 ; i ++ )
	{
		if(distinctLetters[i] != 0 )
		{
			for ( j = 0; j < distinctLetters[i] ; j ++ )
			{
				if( strings[j][digit] == (char) (65 + i ) )
				{
					temp = strings[j];
					for ( k = latestIndex + 1; k <= j; k ++ )
					{
						strings[k] = strings[k-1];
						printf("Infinite Loop\n");
					}
					strings[latestIndex] = temp;
					latestIndex ++;
				}
			}
		}
...
void radixSort2(char **strings, int numberOfLetters, int numberOfStrings )
{
	int digit ,i ;
	for ( digit = numberOfLetters - 1; digit >= 0; digit ++ )
	{
		countSort2(strings,numberOfLetters,numberOfStrings,digit);
	}
} 
int main()
{
...
//	printStrings(output,numberOfLetters,numberOfStrings,"The strings sorted by radixSort1 are");
	...
	radixSort2(strings,numberOfLetters, numberOfStrings);
	printStrings(strings,numberOfLetters, numberOfStrings, "The strings sorted by radixSort2 are");
	return 0;
}
